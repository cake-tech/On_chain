...
class TransactionInstruction {
  TransactionInstruction(
      {required List<AccountMeta> keys, required this.programId, required this.data})
    ...
    try {
      if (programId == AddressLookupTableProgramConst.programId) {
        return AddressLookupTableProgram.fromBytes(
            programId: programId, keys: keys, instructionBytes: instructionBytes);
      } else if (programId == ComputeBudgetConst.programId) {
        return ComputeBudgetProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      } else if (programId == Ed25519ProgramConst.programId) {
        return Ed25519Program.fromBytes(instructionBytes: instructionBytes, keys: keys);
      } else if (programId == MemoProgramConst.latestMemoProgram ||
          programId == MemoProgramConst.v1MemoProgram) {
        return MemoProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      } else if (programId == NameServiceProgramConst.programId) {
        return NameServiceProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      } else if (programId == Secp256k1ProgramConst.programId) {
        return Secp256k1Program.fromBytes(keys: keys, instructionBytes: instructionBytes);
        ...
        return StakeProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      } else if (programId == StakePoolProgramConst.programId) {
        return StakePoolProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      } else if (programId == SystemProgramConst.programId) {
        return SystemProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      } else if (programId == VoteProgramConst.programId) {
        return VoteProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      } else if (programId == TokenLendingProgramConst.lendingProgramId) {
        return TokenLendingProgram.fromBytes(keys: keys, instructionBytes: instructionBytes);
      ...
      return CustomProgram(
          keys: keys, programId: programId, layout: UnknownProgramLayout(instructionBytes));
    }
  }
